...
function likelyVisible(value) {
  ...
  if (/^[\w.-]+\.(?:js|css|json|svg|png|jpe?g|webp|woff2?|ttf|html|mjs)$/i.test(value)) return false;
...
function add(value, file, kind, sample) {
  ...
  if (entry.samples.length < 3 && sample) entry.samples.push(clean(sample).slice(0, 300));
...
function extractHtml(html, file, kind = 'html') {
  ...
  const without = html
    ...
    .replace(/<!--([\s\S]*?)-->/g, ' ');
  for (const match of without.matchAll(/>([^<>]+)</g)) {
    add(match[1], file, `${kind}-text`, match[0]);
  }
}
...
for (const file of files) {
  ...
  const assignment = /\b(textContent|innerText|title|placeholder|ariaLabel|label|description|message|emptyText|helpText)\s*=\s*(["'`])((?:\\.|(?!\2)[\s\S])*)\2/g;
  for (const match of source.matchAll(assignment)) {
    const value = decodeLiteral(match[3], match[2]);
    if (match[1] === 'innerHTML') extractHtml(value, file, 'innerHTML');
    else add(value, file, match[1], match[0]);
  }

  const prop = /\b(label|title|description|subtitle|hint|message|placeholder|empty|note|heading|name|text)\s*:\s*(["'`])((?:\\.|(?!\2)[\s\S])*)\2/g;
...
fs.writeFileSync('interface-string-inventory.json', JSON.stringify({generatedAt:new Date().toISOString(), fileCount:files.length, phraseCount:list.length, phrases:list}, null, 2));
