(() => {
  if (window.__figureLoomDesktopTabRecoveryFinalV1) return;
  window.__figureLoomDesktopTabRecoveryFinalV1 = true;

  const root = document.documentElement;
  let originalAddButton = null;
  let scheduled = false;
  ...
  style.textContent = `
    /* One actual tab box: title, live dot and close X all share this border/background. */
    ...
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap:focus-within>.project-tab-close{
      opacity:1!important;
    }

    /* The real + button sits directly after the final tab in the same strip. */
    ...
    html[data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-scroll>.project-tab-add:hover:not(:disabled){
    ...
    /* Recovery history: same compact desktop density as the approved drawers. */
    ...
    html[data-figureloom-device-class="desktop"] body #historyDrawer .utility-head{
      min-height:44px!important;
      padding:8px 10px!important;
      gap:8px!important;
    }
    html[data-figureloom-device-class="desktop"] body #historyDrawer .utility-head strong{
      font-size:11px!important;
      line-height:1.2!important;
    }
    html[data-figureloom-device-class="desktop"] body #historyDrawer .utility-head span{
    ...
    html[data-figureloom-device-class="desktop"] body #historyDrawer .utility-body{
    ...
    html[data-figureloom-device-class="desktop"] body #historyDrawer .snapshot :where(strong,span){
      font-size:9px!important;
      line-height:1.25!important;
    }
    html[data-figureloom-device-class="desktop"] body #historyDrawer .snapshot small{
      margin-top:2px!important;
    ...
    html[data-figureloom-device-class="desktop"] body #historyDrawer .tool-note{
      font-size:8.5px!important;
      line-height:1.35!important;
    }

    html[data-figureloom-theme="dark"][data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap:hover{
      background:#293440!important;
    }
    html[data-figureloom-theme="dark"][data-figureloom-device-class="desktop"] body #projectTabRail .project-tab-wrap:has(>.project-tab.active){
      border-color:#415064!important;
  ...
  function placeRealAddButton() {
    ...
    if (!originalAddButton) {
      originalAddButton = rail.querySelector('.project-tab-tools>.project-tab-add') ||
        rail.querySelector('.project-tab-scroll>.project-tab-add');
    }
    if (!originalAddButton) return;

    scroll.querySelectorAll('.project-tab-add-inline').forEach(node => {
      if (node !== originalAddButton) node.remove();
    });
    originalAddButton.classList.remove('project-tab-add-inline');
    if (originalAddButton.parentElement !== scroll || scroll.lastElementChild !== originalAddButton) {
      scroll.appendChild(originalAddButton);
    }
  }
  ...
  window.FigureLoomDesktopTabRecoveryFinal = Object.freeze({ refresh, placeRealAddButton, keepStyleLast });
})();