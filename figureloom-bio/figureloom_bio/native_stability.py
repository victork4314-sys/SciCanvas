...
class StableExactWebSyntaxHighlighter(ExactWebSyntaxHighlighter):
    """Web-style coloring that never parses a data or generated-result file."""
...
def _install_worker_explanations(native_ide_module: Any) -> None:
    if getattr(native_ide_module.RunWorker, "_plain_failures_installed", False):
        return
    ...
    def run(worker: Any) -> None:
        try:
            ...
            saved = crash_report("Program", error)
            worker.failed.emit(
                simple_explanation(
                    "The program stopped because FigureLoom Bio hit an internal error.",
                    "The instruction was not skipped and no incomplete result is being presented as finished.",
                    "Save the program, reopen FigureLoom Bio, and run it again. If it repeats, run Repair and keep the crash report below.",
                    technical_detail=f"{error}\n\nCrash report: {saved}",
                ),
                0,
            )

    native_ide_module.RunWorker.run = run
    native_ide_module.RunWorker._plain_failures_installed = True


def install_native_stability(native_ide_module: Any) -> type[Any]:
    """Install startup safety and figure previews without replacing any feature."""

    from . import native_widgets

    native_widgets.NativeSyntaxHighlighter = StableExactWebSyntaxHighlighter
    _install_worker_explanations(native_ide_module)
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                simple_explanation(
                    ...
                    "Run Repair from the updater. If the updater itself will not open, reinstall once from the official download.",
                    technical_detail=f"{error}\n\nCrash report: {saved}",
                ),
            )
            app.processEvents()
